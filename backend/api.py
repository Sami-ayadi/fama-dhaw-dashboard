"""
Famma Dhaw Monitor API - Backend REST API
"""
...
app = FastAPI(
    title="Famma Dhaw Monitor API",
    description="API de monitoring des coupures d'electricite en Tunisie",
    version="1.0.0"
)
...
@app.get("/")
def root():
    return {"message": "Famma Dhaw Monitor API", "version": "1.0.0", "status": "running"}
...
def get_heatmap(hours: int = 24):
    ...
    c.execute("""
        ...
        FROM history
        WHERE timestamp > ?
        GROUP BY governorate, hour
        ORDER BY governorate, hour
    """, (cutoff,))
...
def get_forecast(zone_name: str, hours: int = 6):
    ...
    # Get 7 days of hourly history for this zone
...
def export_csv():
    ...
    for row in rows:
        writer.writerow([row["name"], row["governorate"], row["status"], 
                        row["reports_dark"], row["reports_light"], row["last_updated"]])
